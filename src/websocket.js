...
WebSocketApi.prototype.start = function() {
    ...
    function getPushRules() {
        client.getPushRules().then((result) => {
            ...
            client.syncApi.startKeepAlives().then(function() {
                getPushRules();
            });
    ...
    if (client.isGuest()) {
        ...
        client.store.getSavedSync().then((savedSync) => {
            if (savedSync) {
                return client.syncApi.syncFromCache(savedSync);
            }
        }).then(() => {
...
WebSocketApi.prototype._start = async function(syncOptions) {
    ...
    if (client.isGuest() && !filterId) {
        filterId = client.syncApi.getGuestFilter();
    }
    ...
    try {
        ...
        this._currentSyncRequest = client.syncApi.doSyncRequest(syncOptions, tok);
        data = await this._currentSyncRequest;
    } catch (e) {
        client.syncApi.startKeepAlives().then(() => {
    ...
    try {
        await client.syncApi.processSyncResponse(
            syncEventData, data,
        );
    } catch (e) {
...
WebSocketApi.prototype._start_websocket = function(qps) {
    ...
    function _onclose(ev) {
        ...
        if (self.ws_possible) {
            ...
            self.client.syncApi.startKeepAlives().then(function() {
...
WebSocketApi.prototype.handleSync = async function(data) {
    ...
    await client.store.setSyncData(data);
    try {
        client.syncApi.processSyncResponse(self.ws_syncToken, data, false);
    } catch(e) {
        // log the exception with stack if we have it, else fall back
        // to the plain description
...
WebSocketApi.prototype._onOnline = function() {
    ...
    this.client.syncApi.startKeepAlives(0);
};
